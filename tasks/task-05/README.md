...
print(f"像素范围: [{binary.min()}, {binary.max()}]")
...
| **Sobel** | 计算快速，有方向性 | 边缘较宽，对噪声敏感 | 梯度分析，方向检测 |
...
| - 开运算 | 6分 | 正确实现，说明去噪效果 |
| - 闭运算 | 6分 | 正确实现，说明填充效果 |
| - 形态学梯度 | 6分 | 正确实现，对比边缘检测 |
| - 顶帽/黑帽 | 8分 | 正确实现，说明细节提取 |
| - 综合对比 | 6分 | 创建清晰的对比图 |
| - 原理验证 | 3分 | 手动验证操作公式 |
| **3. 形态学应用** | 20分 | |
| - 去噪实验 | 7分 | 有效去除噪声，对比效果 |
| - 边缘检测对比 | 7分 | 对比三种方法，分析优缺点 |
| - 迭代操作 | 6分 | 实现骨架化和区域增长 |
| **4. 硬币计数** | 20分 | |
| - 完整流程 | 8分 | 预处理、形态学、轮廓检测完整 |
| - 硬币分类 | 6分 | 根据面积正确分类 |
| - 可视化 | 4分 | 清晰标记和统计 |
| - 代码质量 | 2分 | 代码结构清晰，函数封装 |
| **代码质量** | +10分 | |
...
| - 错误处理 | +3分 | 有完善的异常处理机制 |
| **额外加分项** | +20分 | |
| - 分水岭算法 | +8分 | 实现分水岭算法分离粘连物体 |
| - 交互式调参 | +5分 | 使用 Trackbar 调整核大小和形状 |
| - 粘连硬币分离 | +7分 | 实现距离变换和分水岭分离 |

**总分：100分 + 10分（代码质量）+ 20分（额外加分）= 140分**
...
if coins_img is None:
    ...
    cv2.putText(result, f"Total: {len(coins_contours)}", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
